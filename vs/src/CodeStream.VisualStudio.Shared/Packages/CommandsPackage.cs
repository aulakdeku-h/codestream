...
namespace CodeStream.VisualStudio.Packages {
	...
	public sealed class CommandsPackage : AsyncPackage, IServiceContainer, IToolWindowProvider, SToolWindowProvider {
		...
		private static bool TryGetWindowFrame(Guid toolWindowId, out IVsWindowFrame frame) {
			...
			if (!(GetGlobalService(typeof(SVsUIShell)) is IVsUIShell shell) || ErrorHandler.Failed(shell.FindToolWindow((uint)__VSCREATETOOLWIN.CTW_fForceCreate, ref toolWindowId, out frame))) {
				frame = null;
				return false;
			}
